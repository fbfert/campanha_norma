<x-layouts.app title="Destinatarios do lote" breadcrumbs="Inicio / Mensagens / Lotes / Destinatarios">
    ...
    <section class="card" style="margin-top:16px;">
        <div class="table-wrap"><table><thead><tr><th>Posicao</th><th>Nome</th><th>Telefone</th><th>Cidade</th>@if($messageBatch->is_campaign)<th>Modelo sorteado</th>@endif<th>Status</th><th>Motivo</th><th>Mensagem</th></tr></thead><tbody>@foreach($recipients as $recipient)<tr><td>{{ $recipient->random_position }}</td><td>{{ $recipient->contact_name_snapshot }}</td><td>{{ $recipient->contact_phone_snapshot }}</td><td>{{ $recipient->contact_city_snapshot }}</td>@if($messageBatch->is_campaign)<td>{{ $recipient->message_template_name_snapshot ?? '-' }}</td>@endif<td>{{ $recipient->eligibility_status->label() }}</td><td>{{ $recipient->ineligibility_reason }}</td><td><pre style="white-space:pre-wrap;">{{ $recipient->rendered_message }}</pre></td></tr>@endforeach</tbody></table></div>
        {{ $recipients->links() }}
    </section>
</x-layouts.app>
