...
class BatchCreationService
{
    ...
    {
        $body = $this->body($data);
        $this->ensureBodiesAreValid([$body]);
        ...
        return DB::transaction(function () use ($data, $user, $body, $template, $flow, $contacts, $seed, $positions): MessageBatch {
            $batch = MessageBatch::create([
                ...
                'placeholders_snapshot' => $this->placeholdersSnapshot([$body]),
    ...
    public function update(MessageBatch $batch, array $data, User $user): MessageBatch
    {
        ...
        return DB::transaction(function () use ($batch, $data, $user, $body, $flow): MessageBatch {
            $template = filled($data['message_template_id'] ?? null) ? MessageTemplate::findOrFail((int) $data['message_template_id']) : null;
            $contacts = $this->selection->select($data);
    ...
    public function revalidate(MessageBatch $batch, User $user): array
    {
        ...
        DB::transaction(function () use ($batch, &$resultado): void {
            foreach ($batch->recipients()->with('contact')->get() as $recipient) {
                ...
                /*
                 | O texto congelado no lote vence o modelo de hoje.
                 |
                 | Só lote antigo, de quando havia sorteio, tem texto por
                 | destinatário: ali a mensagem real está no modelo sorteado, e
                 | não no corpo do lote. Em lote novo o corpo já é o texto único,
                 | e reler o modelo faria a revalidação adotar uma edição feita
                 | depois que o lote foi preparado.
                 */
                $corpo = $batch->is_campaign && $recipient->template
    ...
    private function writeRecipients(MessageBatch $batch, $contacts, string $body, array $positions): void
    {
        foreach ($contacts as $contact) {
            ...
            $batch->recipients()->create([
                ...
                'message_template_id' => $batch->message_template_id,
                'message_template_version' => $batch->message_template_version,
                'message_template_name_snapshot' => $batch->template?->name,
