 ...
 *
 * Quase todo campo daqui foi digitado por alguém de fora: veio de planilha
 * importada ou do que a própria pessoa informou. Por isso cada célula passa
 * pelo sanitizador de fórmula antes de ser escrita — sem ele, um contato
 * chamado `=HYPERLINK(...)` vira fórmula viva quando a equipe abre o arquivo.
 */
class ContactExportService
{
    ...
    public function export(iterable $contacts, string $format = 'csv'): BinaryFileResponse
    {
        ...
        $writer->addRow(Row::fromValues($this->sanitizer->row(self::HEADERS)));
        $count = 0;

        foreach ($contacts as $contact) {
            $contact->loadMissing('tags');
            $writer->addRow(Row::fromValues($this->sanitizer->row([
                $contact->name,
                $contact->first_name,
                $contact->phone,
                $contact->email,
