<x-layouts.app title="Eventos WhatsApp" breadcrumbs="Inicio / WhatsApp / Eventos">
    <section class="card">
        <form method="get" class="grid grid-3">
            ...
            <div><label for="error_code">Código de erro</label><input id="error_code" name="error_code" value="{{ request('error_code') }}"></div>
            <div><label for="user_id">Usuário</label><select id="user_id" name="user_id"><option value="">Todos</option>@foreach($users as $user)<option value="{{ $user->id }}" @selected((string) request('user_id') === (string) $user->id)>{{ $user->name }}</option>@endforeach</select></div>
    ...
    <section class="card" style="margin-top:16px;">
        <div class="table-wrap">
            <table>
                <thead><tr><th>Data</th><th>Evento</th><th>Status</th><th>Descrição</th><th>Usuário</th><th>Erro</th><th>Detalhes</th></tr></thead>
                <tbody>
