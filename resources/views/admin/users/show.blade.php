<x-layouts.app title="Usuario" breadcrumbs="Inicio / Usuarios / Detalhes">
    @if (session('temporary_password'))
        <div class="alert success"><strong>Senha temporaria:</strong> <code>{{ session('temporary_password') }}</code></div>
    @endif
    <section class="card">
        <h2>{{ $user->name }}</h2>
        ...
        <p><strong>Deve alterar senha:</strong> {{ $user->must_change_password ? 'Sim' : 'Nao' }}</p>
        <p><strong>Ultimo acesso:</strong> {{ $user->last_login_at?->format($dateTimeFormat) ?? 'Sem registro' }}</p>
        <div class="actions">
