<x-layouts.app :title="$document->title" breadcrumbs="Inicio / Base de conhecimento / Base / Documento">
    @if(session('status'))
    ...
    <section class="card">
        <dl class="grid grid-3">
            ...
            <div><dt>Situacao</dt><dd>{{ $document->status->label() }}</dd></div>
            <div><dt>Tipo</dt><dd>{{ $document->type?->label() }}</dd></div>
            <div><dt>Versao</dt><dd>{{ $document->version }}</dd></div>
            ...
            <div><dt>Indexado em</dt><dd>{{ $document->indexed_at?->format($dateTimeFormat) ?: 'Nao indexado' }}</dd></div>
            <div><dt>Aprovado em</dt><dd>{{ $document->approved_at?->format($dateTimeFormat) ?: 'Nao aprovado' }}</dd></div>
        ...
        @if($document->injection_flagged)
            <div class="alert warning">
                <strong>Instrucao detectada neste documento e neutralizada na ingestao.</strong>
                <p class="muted">O trecho suspeito foi substituido antes de virar conteudo recuperavel. Revise o material antes de aprovar.</p>
                <ul>
        ...
        <div class="actions">
            @can('knowledge.approve_documents')
                ...
                <form method="post" action="{{ route('admin.knowledge.documents.reject', [$base, $document]) }}" class="inline">
                    @csrf
                    <input type="text" name="reason" maxlength="255" placeholder="Motivo da rejeicao">
                ...
                <form method="post" action="{{ route('admin.knowledge.documents.obsolete', [$base, $document]) }}" class="inline">
                    @csrf
                    <input type="text" name="reason" maxlength="255" placeholder="Motivo da obsolescencia">
        ...
        <p class="muted">Aprovar e o unico ato que torna este documento alcancavel pela busca. Reprocessar revoga a aprovacao anterior.</p>
    </section>

    <section class="card">
        <h2>Texto extraido</h2>
        <p class="muted">Previa do que a ingestao leu do arquivo, ja com as instrucoes suspeitas neutralizadas.</p>
        <pre class="preformatted">{{ $extractPreview ?: 'Sem texto extraido.' }}</pre>
    </section>

    <section class="card">
        <h2>Trechos indexados</h2>
        <p class="muted">Sao estes os trechos que podem ser citados numa resposta.</p>
        @forelse($chunks as $chunk)
            <article class="card nested">
                <p class="muted">
                    ...
                    @if($chunk->embedded_at) &middot; vetor de {{ $chunk->embedding_dimensions }} dimensoes @endif
                </p>
                <p>{{ $chunk->content }}</p>
            </article>
        @empty
