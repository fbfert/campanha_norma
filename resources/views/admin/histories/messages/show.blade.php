<x-layouts.app title="Detalhe do envio" breadcrumbs="Historicos / Mensagens / Detalhe">
    <section class="card">
        ...
        <p><strong>Erro:</strong> {{ $recipient->error_code ?? '-' }} | <strong>Classificacao:</strong> {{ $classification->label() }}</p>
    </section>
    <section class="card" style="margin-top:16px;">
        ...
        @if($recipient->contact && ($recipient->contact->name !== $recipient->contact_name_snapshot || $recipient->contact->phone !== $recipient->contact_phone_snapshot || $recipient->contact->city !== $recipient->contact_city_snapshot))
            <div class="alert warning">Os dados atuais do contato sao diferentes dos dados utilizados neste envio.</div>
        @endif
    </section>
    <section class="card" style="margin-top:16px;">
        <h2>Mensagem</h2>
        @can('histories.view_message_content')<pre style="white-space:pre-wrap;">{{ $recipient->rendered_message }}</pre>@else<p class="muted">Conteudo protegido por permissao.</p>@endcan
    </section>
    <section class="card" style="margin-top:16px;">
        <h2>Tentativas</h2>
        <div class="table-wrap"><table><thead><tr><th>N</th><th>Status</th><th>Inicio</th><th>Fim</th><th>Erro</th></tr></thead><tbody>@foreach($recipient->attempts()->latest('started_at')->get() as $attempt)<tr><td>{{ $attempt->attempt_number }}</td><td>{{ $attempt->status->value }}</td><td>{{ $attempt->started_at?->format($dateTimeFormat) }}</td><td>{{ $attempt->finished_at?->format($dateTimeFormat) }}</td><td>{{ $attempt->error_code ?? '-' }}</td></tr>@endforeach</tbody></table></div>
    </section>
    <section class="card" style="margin-top:16px;">
        <h2>Eventos</h2>
